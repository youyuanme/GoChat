...
public class User_ScoreAdapter extends BaseAdapter {
	private List<Score> list;
	private Context cxt;

	public User_ScoreAdapter(List<Score> list, Context cxt) {
		this.list = list;
		this.cxt = cxt;
	}
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		Score c = getItem(arg0);
		holder.date.setText(c.getT_AddDate());
		if (c.getType() == 0) {
			holder.type.setText("增加");
			holder.value.setText("+" + c.getT_UserIntegral_Add());
		} else {
			holder.value.setText("+" + c.getT_UserIntegral_Reduce());
